  ...
  render() {
    let recipes = this.props.state.recipes.map(item => {
      return <div className="media my-3" key={item.idMeal} onClick={this.props.onClick}>
                <img className="mr-3 rounded-circle img-fluid shadow-sm" src={item.strMealThumb}  alt={this.props.name}/>
                <div className="media-body col-sm-12">
                    ...
                    <button type="button" className="btn btn-link m-1" data-id={item.id} data-type="prepared"  onClick={this.props.onClick}> <i class="fas fa-check-circle"></i> Mark as prepared</button>
                    ...
                    <button type="button" className="btn btn-outline-warning m-1" data-id={item.id} onClick={()=>this.props.history.push(`/categories/${item.strCategory}/${item.idMeal}`)}><i class="fas fa-clipboard-list"></i> See Detail</button>
                </div>
            </div>
        });
    return (
