...
class FavoriteRecipes extends Component {
  render() {
    let recipes = this.props.favoriteRecipes.map(item => {
      return <div className="media my-3 p-md-3 fav-recipe" key={item.idMeal} onClick={this.props.onClick}>
                ...
                <div className="media-body col-sm-12">
                    ...
                    <button type="button" className="btn btn-outline-success m-1" data-id={item.id} 
                            onClick={()=>this.props.history.push(`/categories/${item.strCategory}/${item.strMeal}/${item.idMeal}`)}><i className="fas fa-clipboard-list"></i> See Detail</button>
                </div>
            </div>
        });
    return (
